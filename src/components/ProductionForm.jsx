...
export default function ProductionForm({ currentUser, supabase }) {
  ...
  // Sign-off State
  const [signOffOperatorId, setSignOffOperatorId] = useState(currentUser?.id || '7222');
  ...
  const [jobs, setJobs] = useState([
    {
      ...
      customerBatchNo: '',
      workpieces: [
        { id: Date.now() + 1, workpieceType: '', quantity: '', unit: 'pcs', weightLb: '', operator: currentUser?.id || '7222' }
      ]
    }
  ]);
  ...
  const addJobRow = () => {
    setJobs([
      ...jobs,
      {
        ...
        customerOrderNo: '',
        customerBatchNo: '',
        workpieces: [{ id: Date.now() + 1, workpieceType: '', quantity: '', unit: 'pcs', weightLb: '', operator: currentUser?.id || '7222' }]
      }
    ]);
  };
  ...
  const addWorkpieceLine = (jobIndex) => {
    const updated = [...jobs];
    ...
    <div className="w-full max-w-7xl mx-auto font-sans text-slate-100 p-2">
      ...
      <div className="flex justify-between items-center mb-4">
        <div>
          ...
          <h1 className="text-2xl font-extrabold text-white">
            New Load Entry & Rigging Sign-off
          </h1>
        </div>
      ...
      <form onSubmit={handleSubmit}>
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          {/* LEFT COLUMN: SOP OPERATING GUIDELINES (5 Cols) */}
          <div className="lg:col-span-5 space-y-4">
            <div className="bg-[#0b1329] border border-slate-800 rounded-2xl p-5 shadow-xl sticky top-4 space-y-4">
              ...
              {/* Dynamic Warning: ONLY shown if wire strands < theoretical value */}
              {isAnyDeficient && (
                <div className="p-4 bg-amber-950/60 border border-amber-800/80 rounded-xl text-amber-200 text-xs leading-relaxed space-y-1.5">
              ...
              <div className="text-xs space-y-4 pt-1">
                ...
                <div className="p-4 bg-[#070d1e] rounded-xl border border-slate-800/80 space-y-1.5">
                  <div className="font-bold text-slate-100 text-xs">1. Tie Wire Knotting</div>
                ...
                <div className="p-4 bg-[#070d1e] rounded-xl border border-slate-800/80 space-y-1.5">
                  <div className="font-bold text-slate-100 text-xs">2. Drainage & Venting Angle</div>
                  <p className="text-[11px] text-slate-400 leading-normal">
          ...
          <div className="lg:col-span-7 space-y-5">
            ...
            {/* 1. Rack & Load ID */}
            <div className="bg-[#0b1329] p-5 rounded-2xl border border-slate-800 shadow-xl grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <select
                  ...
                  className="w-full bg-[#050a17] border border-slate-700/80 rounded-lg px-3 py-2 text-cyan-400 font-mono text-xs focus:outline-none focus:border-cyan-400"
                  required
                >
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full bg-[#050a17] border border-slate-700/80 rounded-lg px-3 py-2 text-slate-400 font-mono text-xs focus:outline-none focus:border-cyan-400"
                  required
                />
              </div>
            </div>

            {/* 2. Loaded Material Breakdown */}
            <div className="bg-[#0b1329] p-5 rounded-2xl border border-slate-800 shadow-xl space-y-4">
              <div className="flex justify-between items-center pb-2">
                ...
                <button
                  type="button"
                  onClick={addJobRow}
                  className="px-3 py-1 bg-[#101b38] hover:bg-[#16244a] text-cyan-400 text-xs font-bold rounded-lg border border-slate-700/80 transition-all cursor-pointer"
                >
                  + Add Another Job
                </button>
              </div>
              ...
              {jobs.map((job, jobIndex) => (
                <div key={job.id} className="bg-[#070d1e] p-4 rounded-xl border border-slate-800/80 space-y-3">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                    <div>
                      <label className="block text-[11px] text-slate-300 mb-1">Customer Name <span className="text-rose-400">*</span></label>
                      <input
                        type="text"
                        placeholder="e.g. ABC Steel / West Coast Fab"
                        value={job.customerName}
                        onChange={(e) => handleJobFieldChange(jobIndex, 'customerName', e.target.value)}
                        className="w-full bg-[#0b1329] border border-slate-800 rounded px-3 py-1.5 text-xs text-slate-100"
                        required
                      />
                    </div>
                    <div>
                      <label className="block text-[11px] text-slate-300 mb-1">Customer Order # <span className="text-rose-400">*</span></label>
                      <input
                        type="text"
                        placeholder="e.g. 102384"
                        value={job.customerOrderNo}
                        onChange={(e) => handleJobFieldChange(jobIndex, 'customerOrderNo', e.target.value)}
                        className="w-full bg-[#0b1329] border border-slate-800 rounded px-3 py-1.5 text-xs font-mono text-cyan-300"
                        required
                      />
                    </div>
                    <div>
                      <label className="block text-[11px] text-slate-300 mb-1">Customer Batch # <span className="text-rose-400">*</span></label>
                      <input
                        type="text"
                  ...
                  <div className="space-y-2 pt-2">
                    <div className="flex justify-between items-center">
                      ...
                      <button
                        type="button"
                        onClick={() => addWorkpieceLine(jobIndex)}
                    ...
                    {job.workpieces.map((wp, wpIndex) => (
                      <div key={wp.id} className="grid grid-cols-12 gap-2 bg-[#0b1329] p-2.5 rounded-lg border border-slate-800/80 items-end">
                        <div className="col-span-3">
                          <label className="block text-[10px] text-slate-400 mb-1">Workpiece Type <span className="text-rose-400">*</span></label>
                          <select
                            value={wp.workpieceType}
                            onChange={(e) => handleWorkpieceChange(jobIndex, wpIndex, 'workpieceType', e.target.value)}
                            className="w-full bg-[#070d1e] border border-slate-700/80 rounded px-2 py-1 text-xs text-slate-100"
                            required
                          >
                        ...
                        <div className="col-span-2">
                          <label className="block text-[10px] text-slate-400 mb-1">Qty <span className="text-rose-400">*</span></label>
                          <input
                            type="number"
                            min="1"
                            value={wp.quantity}
                            onChange={(e) => handleWorkpieceChange(jobIndex, wpIndex, 'quantity', e.target.value)}
                            className="w-full bg-[#070d1e] border border-slate-700/80 rounded px-2 py-1 text-xs text-slate-100 [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
                            required
                          />
                        </div>
                        ...
                        <div className="col-span-2">
                          ...
                          <select
                            ...
                            className="w-full bg-[#070d1e] border border-slate-700/80 rounded px-2 py-1 text-xs font-mono text-slate-100"
                          >
                        ...
                        <div className="col-span-3">
                          <label className="block text-[10px] text-amber-400 font-bold mb-1">Weight (lb) <span className="text-rose-400">*</span></label>
                          <input
                            type="number"
                            step="any"
                            value={wp.weightLb}
                            onChange={(e) => handleWorkpieceChange(jobIndex, wpIndex, 'weightLb', e.target.value)}
                            className="w-full bg-[#070d1e] border border-amber-500/80 rounded px-2 py-1 text-xs font-mono text-amber-300 font-bold [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
                            required
                          />
                        </div>
                        ...
                        <div className="col-span-2">
                          ...
                          <input
                            ...
                            value={wp.operator}
                            readOnly
                            className="w-full bg-[#070d1e] border border-slate-800 rounded px-2 py-1 text-xs font-mono text-cyan-400 font-bold"
                          />
                        </div>
                      </div>
                    ))}
            ...
            {/* 3. Operator Rigging & Hanging Method */}
            <div className="bg-[#0b1329] p-5 rounded-2xl border border-slate-800 shadow-xl space-y-4">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <button
                    ...
                    className={`py-2 px-3 rounded-lg text-xs font-bold border transition-all cursor-pointer flex items-center justify-center gap-1.5 ${
                      hangingPoints === '1' 
                        ? 'bg-[#002b36] border-cyan-500 text-cyan-300' 
              ...
              {/* Point #1 Setup */}
              <div className="p-3.5 bg-[#070d1e] rounded-xl border border-slate-800/80 space-y-3">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div>
                    ...
                    <select
                      ...
                      className="w-full bg-[#0b1329] border border-slate-700/80 rounded px-2.5 py-1.5 text-xs text-slate-100"
                      required
                    >
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className="w-full bg-[#0b1329] border border-slate-700/80 rounded px-2.5 py-1.5 text-xs font-mono text-cyan-300 font-bold [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
                      required
                    />
                  </div>
                </div>
            ...
            <button
              type="submit"
              className="w-full py-3.5 bg-cyan-400 hover:bg-cyan-300 text-slate-950 font-extrabold text-xs uppercase tracking-wider rounded-xl shadow-lg shadow-cyan-400/10 transition-all cursor-pointer mt-2"
            >
