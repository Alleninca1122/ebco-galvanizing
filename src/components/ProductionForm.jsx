...
export default function ProductionForm({ currentUser, supabase }) {
  ...
  const handleRackSelect = async (selectedVal) => {
    ...
    if (selectedVal) {
      ...
      setLoadId(generated); // Sets default generated ID, but user can edit
  ...
  const handleSubmit = async (e) => {
    ...
    if (!rackNo || !loadId.trim()) {
      alert('Please select a Rack/Method and specify a valid Load ID.');
      return;
    }
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="bg-slate-950 p-5 rounded-xl border border-cyan-800/60 relative overflow-hidden">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            ...
            {/* Editable Load ID (Auto-generated by default, editable by operator) */}
            <div>
              ...
              <input
                type="text"
                placeholder={isGeneratingLoadId ? "Generating Load ID..." : "Auto-generated or enter custom ID"}
                value={loadId}
              ...
              <span className="text-[10px] text-slate-500 block mt-1">
                Auto-generated by default. You can manually edit or override this ID if required.
              </span>
            </div>
          </div>
        </div>
